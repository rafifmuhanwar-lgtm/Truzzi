...
export default function RegisterScreen() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (phone.trim().length < 10) return enqueueSnackbar('Nomor telepon tidak valid', { variant: 'error' });
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 flex flex-col max-w-md w-full mx-auto px-6 pt-6 pb-10">
        ...
        <p className="text-sm text-ink-secondary mt-1">Buat akun baru untuk mulai menggunakan Truzzi.</p>

        <form onSubmit={submit} className="mt-8 space-y-4">
          {field(
            'Nama Lengkap',
            <User className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-secondary" />,
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nama kamu" className="input-base pl-10" autoComplete="name" />,
          )}
          {field(
            'Email',
            <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-secondary" />,
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="nama@email.com" className="input-base pl-10" autoComplete="email" />,
          )}
          {field(
            'Nomor Telepon',
            <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-secondary" />,
            <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="08xxxxxxxxxx" className="input-base pl-10" autoComplete="tel" />,
          )}
          <div>
            ...
            <div className="relative">
              ...
              />
              <button type="button" onClick={() => setShowPass((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2" aria-label="Tampilkan password">
          ...
          <button type="button" onClick={() => setAgreed((v) => !v)} className="flex items-start gap-2 text-left w-full">
            <span
