...
import { ArrowLeft, Check, Phone, MapPin, Bike, ShoppingBag, User as UserIcon } from '../components/icons';
...
export default function Tracking() {
  ...
  const [jastiper, setJastiper] = useState<{ name?: string; phone?: string; photoUrl?: string } | null>(null);
  ...
  const statusLabel = order.status === 'completed'
    ...
    : order.statusText || (hasJastiper ? 'Jastiper sedang menuju lokasi belanja' : 'Sedang mencari jastiper terdekat...');
      ...
      <div className="flex-1 max-w-lg w-full mx-auto px-5 py-4 space-y-4 pb-10">
        ...
        <div className={`card-pad ${hasJastiper ? 'border-primary/30' : ''}`}>
          <div className="flex items-center gap-3">
            {order.jastiperAvatar || jastiper?.photoUrl ? (
              <img src={order.jastiperAvatar || jastiper?.photoUrl} alt="" className="w-[52px] h-[52px] rounded-full object-cover" />
            ) : (
              <span className="w-[52px] h-[52px] rounded-full bg-primary/10 flex items-center justify-center">
                {hasJastiper ? <Bike className="w-6 h-6 text-primary" /> : <UserIcon className="w-6 h-6 text-primary" />}
              </span>
            )}
            <div className="flex-1 min-w-0">
              <p className="font-bold">{jastiper?.name || order.jastiperName || 'Mencari Jastiper...'}</p>
              <p className="text-xs text-ink-secondary mt-0.5">{statusLabel}</p>
              {jastiper?.phone && <p className="text-xs text-primary mt-0.5 flex items-center gap-1"><Phone className="w-3 h-3" /> {jastiper.phone}</p>}
            </div>
            {order.status === 'completed' ? (
              <span className="px-2 py-1 rounded-lg bg-success/10 text-success text-xs font-bold">Selesai ✓</span>
            ) : (
              <span className="px-2 py-1 rounded-lg bg-primary/8 text-primary text-xs font-bold">{order.estimasiWaktu ?? '~30 min'}</span>
            )}
          </div>
        </div>
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div key={s.title} className="flex gap-3 relative pb-4 last:pb-0">
                {i < steps.length - 1 && (
                  <div className={`absolute left-4 top-8 bottom-0 w-0.5 ${done ? 'bg-primary' : 'bg-border'}`} />
                )}
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 ${
                  done ? 'bg-primary text-white' : current ? 'bg-primary/20 text-primary border-2 border-primary' : 'bg-surface border border-border text-ink-secondary'
                }`}>
                  <Icon className="w-4 h-4" />
                </div>
                <div className="flex-1 min-w-0 pt-0.5">
                  <p className={`text-xs font-bold ${done ? 'text-ink' : 'text-ink-secondary'}`}>{s.title}</p>
        ...
        {(order.strukImageUrl || order.deliveryProofUrl) && (
          <div className="card-pad border-primary/20 space-y-3">
            ...
            {order.strukImageUrl && (
              <div>
                <p className="text-xs text-ink-secondary mb-1.5 font-medium">Foto Barang yang Dibeli Jastiper</p>
                <img
            ...
            {order.deliveryProofUrl && (
              <div>
                <p className="text-xs text-ink-secondary mb-1.5 font-medium">Bukti Penerimaan Barang</p>
                <img
...
const JASTIP_STEPS = [
  ...
  { title: 'Jastiper Menuju Toko', sub: 'Jastiper sedang dalam perjalanan ke lokasi pembelian', icon: Bike },
  { title: 'Sedang Dibelikan Jastiper', sub: 'Jastiper membelikan barang pesanan kamu di toko', icon: ShoppingBag },
  { title: 'Dalam Perjalanan ke Customer', sub: 'Jastiper menuju lokasi pengantaran kamu', icon: Bike },
