...
export default function AdminDashboard() {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'pending' | 'completed' | 'rejected'>(
    'all',
  );
  ...
  const handleCreateBanner = async (e: FormEvent) => {
    ...
    try {
      ...
      enqueueSnackbar('Banner promo resmi dipublikasikan ke Beranda Customer! 🎉', {
        variant: 'success',
      });
      setBannerTitle('');
      setBannerSubtitle('');
  ...
  const handleToggleOnline = async (id: string, currentOnline: boolean) => {
    try {
      await API.admin.updateJastiper(id, {
        isOnline: !currentOnline,
        status: !currentOnline ? 'online' : 'offline',
      });
      enqueueSnackbar(`Status mitra diubah menjadi ${!currentOnline ? 'Online' : 'Offline'}`, {
        variant: 'success',
      });
      refetchJastipers();
      refetchStats();
  ...
  const handleUpdateOrderStatus = async (id: string, newStatus: string) => {
    try {
      await API.admin.updateOrder(id, {
        status: newStatus,
        statusText: `Diperbarui Admin: ${newStatus}`,
      });
    ...
    <div className="min-h-screen bg-[#0F172A] text-slate-100 flex flex-col font-sans antialiased selection:bg-primary/30 selection:text-white">
      ...
      <header className="bg-[#1E293B]/90 backdrop-blur-xl border-b border-slate-800/80 px-6 h-16 sticky top-0 z-40 flex items-center justify-between shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 font-medium">
                Enterprise Management & Real-time Operations Console
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <RefreshCw
              className={`w-3.5 h-3.5 text-primary-light ${fetchingStats || fetchingWd ? 'animate-spin' : ''}`}
            />
      ...
      <div className="bg-[#1E293B]/40 border-b border-slate-800/80 px-6">
        <div className="max-w-7xl mx-auto flex items-center gap-1 overflow-x-auto no-scrollbar">
          ...
          >
            <ShieldCheck className="w-4 h-4 text-red-400" /> Sengketa (
            {orders.filter((o: any) => o.needsAdminReview && !o.adminReviewed).length})
          </button>
          ...
          >
            <ShieldCheck className="w-4 h-4 text-indigo-400" /> Verifikasi KYC (
      ...
      <main className="flex-1 p-6 max-w-7xl w-full mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-[#1E293B]/80 border border-slate-800 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-700 transition-all shadow-lg">
                ...
                <p className="text-2xl font-black text-amber-400 tracking-tight">
                  {formatRupiah(stats?.totalEscrowLocked ?? 0)}
                </p>
                <div className="mt-3 pt-3 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-400 font-medium">
              ...
              <div className="bg-[#1E293B]/80 border border-slate-800 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-700 transition-all shadow-lg">
                ...
                <p className="text-2xl font-black text-white tracking-tight">
                  {stats?.completedOrdersCount ?? 0} Transaksi
                </p>
                <div className="mt-3 pt-3 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-400 font-medium">
              ...
              <div className="bg-[#1E293B]/80 border border-slate-800 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-700 transition-all shadow-lg">
                ...
                <p className="text-2xl font-black text-white tracking-tight">
                  {stats?.activeJastipersCount ?? 0}{' '}
                  <span className="text-xs font-semibold text-slate-400">
                    / {stats?.totalJastipersCount ?? 0} Mitras
                  </span>
                </p>
                <div className="mt-3 pt-3 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-400 font-medium">
                ...
                <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                  <div>
                    <h3 className="font-bold text-sm text-white">
                      Status Operasional & Tugas Cari Cuan
                    </h3>
                    <p className="text-xs text-slate-400">
                      Pengawasan layanan antar customer dan tugas antar customer (Cari Cuan).
                    </p>
                  </div>
                ...
                <div className="space-y-2.5">
                  ...
                  >
                    <div>
                      <p className="text-xs font-bold text-white group-hover:text-primary-light transition-colors">
                        🖼️ Upload Banner Beranda
                      </p>
                      <p className="text-[11px] text-slate-400">
                        Tambahkan promo terbaru untuk customer
                      </p>
                    </div>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400">
                        Proses pencairan dana bank & e-wallet
                      </p>
                    </div>
                    <ChevronRight className="w-4 h-4 text-slate-500 group-hover:text-white transition-colors" />
                  </button>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-[#1E293B]/60 p-4 rounded-2xl border border-slate-800">
              <div>
                <h2 className="font-bold text-sm text-white">
                  Kelola & Pantau Semua Pesanan ({orders.length})
                </h2>
                <p className="text-[11px] text-slate-400">
                  Ubah status alur pengiriman atau hapus data pesanan.
                </p>
              </div>
            </div>
                      ...
                      {orders.map((o) => (
                        <tr key={o.id || o.$id} className="hover:bg-slate-800/40 transition-colors">
                          <td className="p-3.5">
                            ...
                            <span className="text-[10px] font-bold text-primary-light uppercase">
                              {o.serviceName || o.orderType}
                            </span>
                          </td>
                          ...
                          <td className="p-3.5 text-right">
                            <div className="inline-flex items-center gap-2 justify-end">
                              <select
                                value={o.status}
                                onChange={(e) =>
                                  handleUpdateOrderStatus(o.id || o.$id, e.target.value)
                                }
        ...
        {activeTab === 'jastipers' && (
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-[#1E293B]/60 p-4 rounded-2xl border border-slate-800">
              <div>
                <h2 className="font-bold text-sm text-white">
                  Mitra Jastiper & Driver Lapangan ({jastipers.length})
                </h2>
                <p className="text-[11px] text-slate-400">
                  Kelola status online/offline mitra, rating, dan verifikasi akun.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {jastipers.map((c) => (
                <div
                  key={c.id || c.$id}
                  className="bg-[#1E293B]/80 border border-slate-800 rounded-2xl p-4 space-y-3 relative overflow-hidden shadow-lg"
                >
                  <div className="flex items-center gap-3">
                    {c.photoUrl ? (
                      <img
                  ...
                  <div className="p-2.5 rounded-xl bg-[#0F172A] flex items-center justify-between text-[11px]">
                    ...
                    <span
                      className={`font-bold ${c.isOnline ? 'text-emerald-400' : 'text-slate-500'}`}
                    >
        ...
        {activeTab === 'users' && (
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-[#1E293B]/60 p-4 rounded-2xl border border-slate-800">
              <div>
                <h2 className="font-bold text-sm text-white">
                  Data Pengguna Terdaftar ({users.length})
                </h2>
                <p className="text-[11px] text-slate-400">
                  Daftar customer aktif di aplikasi Truzzi.
                </p>
              </div>
            </div>
        ...
        {activeTab === 'gigs' && (
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-[#1E293B]/60 p-4 rounded-2xl border border-slate-800">
              <div>
                <h2 className="font-bold text-sm text-white">
                  Tugas Komunitas Cari Cuan ({gigs.length})
                </h2>
                <p className="text-[11px] text-slate-400">
                  Moderasi tugas & pekerjaan freelance yang diposting sesama pengguna.
                </p>
              </div>
            </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-black text-emerald-400">
                      {formatRupiah(g.reward || g.price || 0)}
                    </span>
                  </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1 bg-[#1E293B]/80 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-xl">
                <div className="border-b border-slate-800 pb-3">
                  ...
                  <p className="text-[11px] text-slate-400 mt-1">
                    Banner yang dipublikasikan di sini akan tampil di Carousel halaman Beranda
                    Customer (`HomeScreen.tsx`).
                  </p>
                </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-sm text-white flex items-center gap-2">
                    <Ticket className="w-4 h-4 text-amber-400" /> Daftar Banner Promo Dipublikasikan
                    ({promos.length})
                  </h3>
                </div>
                ...
                {promos.length === 0 ? (
                  <div className="p-12 rounded-2xl border border-slate-800 bg-[#1E293B]/40 text-center space-y-1.5">
                    ...
                    <p className="text-xs font-bold text-slate-300">
                      Belum ada banner promo custom yang aktif
                    </p>
                    <p className="text-[11px] text-slate-500">
                      Gunakan form di sebelah kiri untuk mengunggah dan menerbitkan banner promo
                      baru.
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {promos.map((p) => (
                      <div
                        key={p.id}
                        className="bg-[#1E293B]/80 border border-slate-800 rounded-2xl p-4 flex gap-3.5 relative overflow-hidden group hover:border-slate-700 transition-all shadow-md"
                      >
                        {p.imageUrl && (
                          <img
                            src={p.imageUrl}
                            alt={p.title}
                            className="w-20 h-20 rounded-xl object-cover shrink-0 border border-slate-700/50"
                          />
                        )}
                        <div className="flex-1 space-y-1 min-w-0">
                          ...
                          <p className="text-[10px] font-mono font-extrabold text-emerald-400">
                            Kode: {p.code}
                          </p>
                          <button
        ...
        {activeTab === 'withdrawals' && (
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-[#1E293B]/60 p-4 rounded-2xl border border-slate-800">
              <div>
                <h2 className="font-bold text-sm text-white">
                  Antrean &amp; Verifikasi Penarikan Saldo
                </h2>
                <p className="text-[11px] text-slate-400">
                  Konfirmasi pencairan dana ke Bank (Gratis Admin) &amp; E-Wallet (Admin Rp 2.500).
                </p>
              </div>
        ...
        {activeTab === 'monitoring' && (
          <div className="space-y-4">
            <div className="bg-[#1E293B]/80 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-xl">
              <div>
                <h2 className="font-bold text-sm text-white flex items-center gap-2">
                  <ShieldCheck className="w-5 h-5 text-emerald-400" /> Platform Escrow & Safety
                  Architecture
                </h2>
                <p className="text-xs text-slate-400 mt-1">
                  Truzzi menjamin keamanan transaksi 100% untuk Customer dan Jastiper/Pengerja
                  melalui enkripsi & proteksi escrow.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                <div className="p-4 rounded-xl bg-[#0F172A] border border-slate-800 space-y-1.5">
                  ...
                  <p className="text-[11px] text-slate-400 leading-relaxed">
                    Dana pembayaran tertahan di rekening escrow platform dan baru diteruskan setelah
                    konfirmasi penerimaan pekerjaan.
                  </p>
                </div>
                <div className="p-4 rounded-xl bg-[#0F172A] border border-slate-800 space-y-1.5">
                  ...
                  <p className="text-[11px] text-slate-400 leading-relaxed">
                    Ruang obrolan terpantau untuk memastikan tidak terjadi transaksi ilegal atau
                    pelecehan antar pengguna.
                  </p>
                </div>
                <div className="p-4 rounded-xl bg-[#0F172A] border border-slate-800 space-y-1.5">
                  <span className="text-xs font-bold text-purple-400">
                    📑 Fraud Prevention & Proof Verification
                  </span>
                  <p className="text-[11px] text-slate-400 leading-relaxed">
                    Setiap pengerjaan tugas & pengantaran barang wajib melampirkan foto bukti fisik
        ...
        {activeTab === 'review' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-sm text-white flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-red-400" /> Review Pesanan Nominal Tinggi (≥
                Rp1.000.000)
              </h2>
              <button
            ...
            {orders.filter((o: any) => o.needsAdminReview).length === 0 ? (
              <div className="text-center py-12 text-slate-500 text-sm">
                Tidak ada pesanan yang memerlukan review
              </div>
            ) : (
              <div className="space-y-3">
                {orders
                  .filter((o: any) => o.needsAdminReview)
                  .map((o: any) => (
                    <div
                      key={o.id}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-slate-400">
                            Jastiper: {o.jastiperName || o.jastiperId || '-'}
                          </p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-sm font-bold text-amber-400">
                            {formatRupiah(o.totalAmount ?? 0)}
                          </p>
                          <p
                        ...
                        {o.strukImageUrl && (
                          <div className="space-y-1">
                            ...
                            <img
                              ...
                              onClick={() => window.open(o.strukImageUrl, '_blank')}
                            />
                          </div>
                        )}
                        {o.deliveryProofUrl && (
                          <div className="space-y-1">
                            ...
                            <img
                              ...
                              alt="bukti"
                              className="w-24 h-24 object-cover rounded-lg border border-slate-700 cursor-pointer"
                              onClick={() => window.open(o.deliveryProofUrl, '_blank')}
                            />
                          </div>
                        )}
                      </div>
                          ...
                          <button
                            onClick={async () => {
                              try {
                                ...
                                enqueueSnackbar('Pesanan ditandai sudah direview', {
                                  variant: 'success',
                                });
                                refetchOrders();
                              } catch (err) {
              ...
              {pendingKycJastipers.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {pendingKycJastipers.map((j) => (
                    <div
                      key={j.id || j.$id}
                      className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4 flex flex-col"
                    >
                      ...
                      <div className="mt-auto space-y-3">
                        <div className="flex items-center justify-between text-xs border-t border-slate-700/50 pt-3">
                          ...
                          <span className="text-amber-400 font-bold px-2 py-0.5 bg-amber-400/10 rounded-full">
                            Pending
                          </span>
                        </div>
                        <button
          ...
          <div className="bg-[#1E293B] border border-slate-700 w-full max-w-3xl rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            <div className="p-4 border-b border-slate-700 flex items-center justify-between bg-[#0F172A]">
              <h3 className="font-bold text-white text-lg flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-indigo-400" /> Verifikasi KYC:{' '}
                  ...
                  <div className="aspect-[16/10] bg-slate-900 rounded-xl border border-slate-700 overflow-hidden relative group">
                    ...
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="px-4 py-2 bg-slate-800 rounded-lg text-white text-xs font-bold"
                      >
                        Perbesar
                      </a>
                    </div>
