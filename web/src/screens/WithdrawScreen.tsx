...
export default function WithdrawScreen() {
  ...
  const saldo = Number(wallet?.balance ?? 0) || 0;
  ...
  const rawValue = parseRupiah(amount); // Nominal yang ingin ditarik dari saldo (misal 50.000)
  const netReceived = Math.max(0, rawValue - adminFee); // Jumlah yang masuk ke akun customer (misal 47.500)

  async function handleSubmit() {
    if (!amount.trim()) {
      enqueueSnackbar('Masukkan nominal penarikan', { variant: 'warning' });
      return;
    }
    ...
    try {
      await API.wallet.withdraw({
        amount: value,
        method: selectedItem?.name || selectedId,
        accountNumber: accountNumber.trim(),
        accountName: '',
        type: isEwallet ? 'ewallet' : 'bank',
      } as any);
      enqueueSnackbar('Permintaan penarikan berhasil diproses ', { variant: 'success' });
