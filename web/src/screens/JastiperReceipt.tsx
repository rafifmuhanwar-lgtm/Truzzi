    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 max-w-lg w-full mx-auto px-6 py-5 space-y-4 pb-28">
        ...
        <div className="bg-primary/5 border border-primary/10 rounded-2xl p-4">
          <p className="text-xs font-semibold text-ink-secondary flex items-center gap-1.5"><Info className="w-3.5 h-3.5 text-primary" /> Informasi Pesanan</p>
          <div className="mt-2 space-y-1.5 text-sm">
            <div className="flex justify-between"><span className="text-ink-secondary">Item</span><span className="font-semibold">{order.title}</span></div>
            <div className="flex justify-between"><span className="text-ink-secondary">Dana Belanja</span><span className="font-semibold">{formatRupiah(order.danaBelanja)}</span></div>
            <div className="flex justify-between"><span className="text-ink-secondary">Ongkir</span><span className="font-semibold">{formatRupiah(order.ongkir)}</span></div>
          </div>
        </div>
        ...
        <div className="card-pad">
          ...
          <p className="text-xs text-ink-secondary mt-0.5">Masukkan nominal sesuai struk belanja dari toko</p>
          ...
          >
            {photo ? (
              <img src={photo} alt="foto struk" className="w-full h-full object-cover rounded-2xl" />
            ) : (
              <>
          ...
          <div className="relative mt-4">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-bold text-ink-secondary">Rp</span>
            <input
              value={amount}
        ...
        {isJanganLebih && (
          <div className="bg-warning/10 border border-warning/25 rounded-2xl p-3.5 flex items-start gap-2.5">
            ...
            <p className="text-xs leading-relaxed">
              <span className="font-bold">Kebijakan: Jangan melebihi dana belanja.</span>{' '}
              Kurangi barang jika total melebihi {formatRupiah(order.danaBelanja)}.
            </p>
          </div>
        )}
      </div>
