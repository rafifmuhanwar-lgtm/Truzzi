...
import { ArrowLeft, MapPin, Clock, Monitor, Wrench, X, Star, MessageCircle } from '../components/icons';
...
export default function GigDetailScreen() {
  ...
  if (!gig) {
    return (
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center text-sm text-ink-secondary">Memuat...</div>
      </div>
    );
  }
  ...
  const doCancel = async () => {
    if (!window.confirm('Yakin ingin membatalkan tugas ini? Dana akan dikembalikan ke saldomu.')) return;
    try {
  ...
  const showChat = (isPoster || isWorker) && gig.workerId && (isInProgress || isSubmitted || isCompleted);
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 max-w-lg w-full mx-auto px-6 py-5 space-y-4 pb-32">
        ...
        <div className="flex items-center gap-2">
          <span className={`px-2.5 py-1 rounded-lg text-[11px] font-bold ${statusColor(gig.status)}`}>
          ...
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-background text-[11px] font-semibold text-ink-secondary">
            {gig.category === 'digital' ? <Monitor className="w-3.5 h-3.5" /> : <Wrench className="w-3.5 h-3.5" />}
        ...
        <div className="card p-5">
          ...
          <p className="text-sm text-ink-secondary mt-2 whitespace-pre-line leading-relaxed">{gig.description}</p>

          <div className="flex items-center gap-3 mt-4 text-xs text-ink-secondary">
            <span className="inline-flex items-center gap-1"><Clock className="w-4 h-4 text-primary" />{formatDate(gig.createdAt)}</span>
            {gig.deadline && <span className="inline-flex items-center gap-1"><Clock className="w-4 h-4 text-warning" />Deadline {formatDate(gig.deadline)}</span>}
          </div>
          {gig.location && (
        ...
        <div className="card p-4 border border-primary/20">
          <div className="flex justify-between text-sm"><span className="text-ink-secondary">Budget pengerja</span><span className="font-semibold">{formatRupiah(gig.budget)}</span></div>
          <div className="flex justify-between text-sm mt-1"><span className="text-ink-secondary">Biaya layanan (2,1%)</span><span className="font-semibold">{formatRupiah(gig.biayaLayanan)}</span></div>
          <div className="h-px bg-divider my-2.5" />
          <div className="flex justify-between items-center">
            <span className="font-bold">Total escrow</span>
            <span className="font-bold text-primary">{formatRupiah((gig.budget || 0) + (gig.biayaLayanan || 0))}</span>
          </div>
        </div>
        ...
        {isSubmitted && gig.proofNote && (
          <div className="card p-4">
            ...
            {gig.proofImageUrl && (
              <img src={gig.proofImageUrl} alt="Bukti" className="w-full rounded-xl border border-border mb-2 object-cover max-h-[240px]" />
            )}
            ...
            {reviews.map((r) => (
              <div key={r.id ?? r.$id} className="flex items-start gap-2 py-1.5">
                <div className="flex gap-0.5">
                  {[1, 2, 3, 4, 5].map((n) => (
                    <Star key={n} className={`w-3.5 h-3.5 ${n <= r.rating ? 'text-warning fill-current' : 'text-border'}`} />
                  ))}
                </div>
      ...
      {(showTake || showCancel || showSubmit || showApprove || showReview || showChat) && (
        <div className="fixed bottom-0 left-0 right-0 bg-white px-6 pt-4 pb-6 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-20">
          <div className="max-w-lg mx-auto space-y-2">
            ...
            {showTake && <button onClick={doTake} className="btn-primary">Ambil Tugas Ini</button>}
            {showSubmit && <button onClick={() => setSubmitOpen(true)} className="btn-primary">Kirim Bukti Pengerjaan</button>}
            {showApprove && <button onClick={doApprove} className="btn-primary">Setujui & Lepas Dana</button>}
            {showCancel && <button onClick={doCancel} className="btn-outline">Batalkan Tugas</button>}
            {showReview && (
              <button onClick={() => setReviewOpen(true)} className="btn-primary flex items-center justify-center gap-2">
...
function statusColor(s: string) {
  switch (s) {
    ...
    case 'in_progress': return 'bg-[#E3F2FD] text-[#1565C0]';
    case 'submitted': return 'bg-[#FFF3E0] text-[#E65100]';
    case 'completed': return 'bg-[#E8F5E9] text-[#2E7D32]';
    case 'cancelled': return 'bg-[#FFEBEE] text-[#C62828]';
    default: return 'bg-border/50 text-ink-secondary';
  }
}
...
function SubmitModal({ onClose, onSubmit }: {
  ...
  return (
    <div className="fixed inset-0 z-[1500] bg-black/40 flex items-end justify-center" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="bg-white w-full max-w-lg rounded-t-3xl p-6" style={{ animation: 'slideUp 0.25s ease-out' }}>
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold">Kirim Bukti</h3>
          <button onClick={onClose} aria-label="Tutup"><X className="w-5 h-5 text-ink-secondary" /></button>
        </div>
        <p className="text-xs text-ink-secondary mt-1">Unggah foto/dokumen hasil pekerjaan untuk ditinjau pemasang tugas.</p>

        <div className="mt-4">
          <label className="text-xs font-medium mb-1.5 block text-ink-secondary">Foto Bukti (Opsional)</label>
        ...
        <div className="mt-4">
          ...
          <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={3} placeholder="Jelaskan hasil pekerjaanmu..." className="input-base" />
        </div>

        <button onClick={() => onSubmit(imageData, note)} className="btn-primary mt-5">Kirim Bukti</button>
      </div>
    </div>
  );
}

function ReviewModal({ rating, setRating, comment, setComment, onClose, onSubmit }: {
  rating: number;
        ...
        <div className="flex gap-1 mt-4 justify-center">
          {[1, 2, 3, 4, 5].map((n) => (
            <button key={n} onClick={() => setRating(n)} aria-label={`${n} bintang`}>
              <Star className={`w-9 h-9 ${n <= rating ? 'text-warning fill-current' : 'text-border'}`} />
            </button>
          ))}
        </div>

        <div className="mt-4">
          <label className="text-xs font-medium mb-1.5 block text-ink-secondary">Komentar (Opsional)</label>
          <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={3} className="input-base" placeholder="Bagaimana hasilnya?" />
        </div>

        <button onClick={onSubmit} className="btn-primary mt-5">Kirim Penilaian</button>
      </div>
    </div>
  );
}
