...
export function VoucherPickerSheet({ isOpen, onClose, claims, onSelect, selectedCode }: VoucherPickerSheetProps) {
    ...
    <div className="fixed inset-0 z-[1500] bg-black/40 flex items-end justify-center" onClick={onClose}>
      ...
      >
        ...
        <div className="p-4 bg-white border-b border-border shrink-0 flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-xs text-ink-secondary font-semibold">Tutup</button>
        </div>
        ...
        <div className="p-4 overflow-y-auto space-y-3 bg-slate-50">
          {claims.length === 0 ? (
            <div className="py-10 text-center">
              ...
              <p className="text-sm font-semibold text-ink-secondary">Yah, belum ada voucher nih :(</p>
              <p className="text-xs text-ink-secondary/70 mt-1">Klaim voucher di halaman depan ya!</p>
            </div>
          ) : (
            claims.map((c: any) => {
              const p = c.promo;
