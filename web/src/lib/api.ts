...
export const API = {
  auth: {
    register: (data: { name: string; email: string; password: string; phone?: string; selectedArea?: string }) =>
    ...
    googleCallback: (code: string) => api.post('/auth/google/callback', { code }).then((r) => r.data),
    me: () => api.get('/auth/me').then((r) => r.data),
    logout: () => api.post('/auth/logout').then((r) => r.data),
    updateProfile: (data: { name?: string; phone?: string; photoUrl?: string; selectedArea?: string }) =>
      api.put('/auth/profile', data).then((r) => r.data),
  },
  jastipers: {
    list: (params?: { area?: string; sort?: string; search?: string; lat?: number; lng?: number }) =>
      api.get('/jastipers', { params }).then((r) => r.data),
    get: (id: string) => api.get(`/jastipers/${id}`).then((r) => r.data),
    nearBy: (lat: number, lng: number, radius?: number) =>
      api.get('/jastipers/nearby', { params: { lat, lng, radius: radius ?? 5 } }).then((r) => r.data),
  ...
  favorites: {
    ...
    isFavorite: (jastiperId: string) => api.get(`/favorites/check/${jastiperId}`).then((r) => r.data),
  },
  orders: {
    ...
    patch: (id: string, data: Record<string, unknown>) => api.patch(`/orders/${id}`, data).then((r) => r.data),
    jastiper: (id: string) => api.get(`/orders/${id}/jastiper`).then((r) => r.data),
    escrow: (id: string, data?: Record<string, unknown>) => api.post(`/orders/${id}/escrow`, data ?? {}).then((r) => r.data),
    report: (id: string, data: Record<string, unknown>) => api.post(`/orders/${id}/report`, data).then((r) => r.data),
    review: (id: string, data: Record<string, unknown>) => api.post(`/orders/${id}/review`, data).then((r) => r.data),
    confirmReceived: (id: string, data: { rating: number; reviewText: string }) => api.post(`/jastiper/orders/${id}/confirm-received`, data).then((r) => r.data),
  },
  jastipProducts: {
  ...
  chat: {
    ...
    generateResi: (orderId: string) => api.post(`/orders/${orderId}/generate-resi`).then((r) => r.data),
    editOrder: (orderId: string, data: Record<string, unknown>) => api.patch(`/orders/${orderId}`, data).then((r) => r.data),
  ...
  wallet: {
    ...
    topup: (data: { amount: number; method?: string }) => api.post('/topup', data).then((r) => r.data),
    withdraw: (data: { amount: number; method: string; accountNumber: string; accountName?: string }) =>
  ...
  location: {
    ...
    reverseGeocode: (lat: number, lng: number) => api.get('/reverse-geocode', { params: { lat, lng } }).then((r) => r.data),
    ...
    places: (q: string, lat?: number, lng?: number) =>
      api.get('/places/search', { params: { q, ...(lat !== undefined ? { lat } : {}), ...(lng !== undefined ? { lng } : {}) } }).then((r) => r.data),
  },
  promos: {
  ...
  gigs: {
    ...
    submit: (id: string, data: Record<string, unknown>) => api.post(`/gigs/${id}/submit`, data).then((r) => r.data),
    approve: (id: string) => api.post(`/gigs/${id}/approve`).then((r) => r.data),
    cancel: (id: string) => api.post(`/gigs/${id}/cancel`).then((r) => r.data),
    review: (id: string, data: Record<string, unknown>) => api.post(`/gigs/${id}/review`, data).then((r) => r.data),
  },
  notifications: {
  ...
  addresses: {
    ...
    update: (id: string, data: Record<string, unknown>) => api.patch(`/addresses/${id}`, data).then((r) => r.data),
  ...
  admin: {
    ...
    updateOrder: (id: string, data: Record<string, unknown>) => api.patch(`/admin/orders/${id}`, data).then((r) => r.data),
