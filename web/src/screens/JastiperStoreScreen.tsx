...
import { ArrowLeft, Share2, Star, CheckCircle2, Clock, MessageCircle, Minus, Plus, ShoppingBag, X, ChevronLeft, ChevronRight } from '../components/icons';
...
export default function JastiperStoreScreen() {
  ...
  if (loading) {
    return <div className="min-h-screen bg-background flex items-center justify-center">Memuat data...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-background pb-32">
      ...
      <header className="bg-surface px-4 py-3 flex items-center justify-between sticky top-0 z-20 shadow-sm border-b border-border">
        <button onClick={() => navigate(-1)} aria-label="Kembali" className="w-8 h-8 rounded-full bg-surface-hover flex items-center justify-center shrink-0">
          <ArrowLeft className="w-5 h-5 text-ink" />
        </button>
        <h1 className="font-semibold text-base flex-1 ml-3 truncate">Toko {jastiper.name}</h1>
        <button onClick={handleShare} aria-label="Bagikan" className="w-8 h-8 rounded-full bg-surface-hover flex items-center justify-center shrink-0">
      ...
      <div className="bg-surface px-4 py-5 border-b border-border">
        <div className="flex items-start gap-4">
          {jastiper.photoUrl ? (
            <img src={jastiper.photoUrl} alt="" className="w-16 h-16 rounded-2xl object-cover border border-border shrink-0 shadow-sm" />
          ) : (
        ...
        <div className="mt-5 bg-gradient-to-r from-primary/5 to-transparent border border-primary/20 rounded-2xl p-3.5">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] font-bold text-primary uppercase tracking-wider">JADWAL TRIP JASTIP</span>
            <span className="text-xs font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full">{jastiper.openTripSchedule || 'Besok Pagi'}</span>
          </div>
          <p className="text-sm font-bold text-ink mb-1">{jastiper.openTripTitle || `Jastip ${jastiper.name}`}</p>
          <p className="text-xs text-ink-secondary leading-relaxed">{jastiper.bio || 'Siap membantu membelikan barang titipanmu.'}</p>
          <div className="mt-2 flex items-center gap-1.5 text-error text-xs font-medium">
            <Clock className="w-3.5 h-3.5" /> Batas Pemesanan: {jastiper.openTripClosing || 'Hari ini'}
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-5">
        ...
        {jastiper.products && jastiper.products.length > 0 ? (
            ...
            {jastiper.products.map((p: any) => {
              ...
              >
                  {p.images && p.images.length > 0 ? (
                    <img src={p.images[0]} alt={p.title} className="w-20 h-20 rounded-xl object-cover shrink-0 border border-border/60" />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0 py-0.5">
                    ...
                    <p className="text-xs text-ink-secondary line-clamp-2 leading-relaxed mt-1">{p.description}</p>
                    <div className="flex items-center justify-between mt-2">
                      <p className="text-sm font-bold text-primary">{formatRupiah(p.price)}</p>
                      {/* Qty Selector */}
                      <div className="flex items-center gap-2 shrink-0 bg-[#F3F4F6] border border-border/60 rounded-xl p-1" onClick={(e) => e.stopPropagation()}>
                        <button
                          onClick={() => updateCart(p.id, -1)}
                          disabled={qty === 0}
                          className={`w-7 h-7 rounded-lg flex items-center justify-center transition-colors ${
                            qty > 0 ? 'bg-white text-ink shadow-2xs active:scale-95' : 'text-ink-secondary/30'
                          }`}
                        >
          ...
          <div className="bg-surface border border-border rounded-2xl p-6 text-center">
            ...
            <p className="text-xs text-ink-secondary mt-1">Jastiper ini belum mengunggah barang apa pun.</p>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-[480px] mx-auto bg-surface border-t border-border p-4 pb-8 shadow-[0_-4px_15px_-3px_rgba(0,0,0,0.05)] z-20 space-y-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-lg font-bold text-primary leading-none mt-0.5">{formatRupiah(calculateTotalOrder())}</p>
          </div>
        </div>
      ...
      {selectedProduct && (() => {
        ...
        return (
          <div className="fixed inset-0 z-50 flex flex-col" onClick={() => setSelectedProduct(null)}>
            {/* Backdrop */}
              ...
              <div className="border-t border-border px-5 py-4 flex items-center gap-3 bg-surface">
                <div className="flex items-center gap-2 bg-[#F3F4F6] border border-border/60 rounded-xl p-1">
                  <button
                    ...
                    className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${
                      qty > 0 ? 'bg-white text-ink shadow-sm active:scale-95' : 'text-ink-secondary/30'
                    }`}
                  >
                ...
                <button
                  onClick={() => {
                    ...
                    enqueueSnackbar(`${p.title} ditambahkan ke keranjang`, { variant: 'success' });
                  }}
