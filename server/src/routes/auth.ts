  ...
  try {
    ...
    if (config.data.engine === 'postgres') {
      ...
      return res
        .status(201)
        .json({
          user: { id: created.id, name: created.name, email: created.email, phone: created.phone },
          engine: 'postgres',
        });
    }
  } catch (e: any) {
  ...
  try {
    ...
    if (config.data.engine === 'postgres') {
      ...
      if (!userRow.passwordHash)
        return res
          .status(401)
          .json({ message: 'Login Gagal: Akun ini tidak memiliki password (mungkin via Google)' });
      ...
      const user: SessionUser = {
        ...
        selectedArea: userRow.selectedArea,
        role: userRow.role === 'jastiper' ? 'jastiper' : 'customer',
      };
...
router.post('/google/callback', async (req: Request, res: Response) => {
  try {
    ...
    if (!tokenData.access_token)
      return res.status(401).json({ message: 'Gagal menukar code Google' });
    ...
    const user: SessionUser = {
      ...
      email: row.email,
      phone: row.phone,
      photoUrl: row.photoUrl ?? info.picture ?? null,
...
router.get('/me', async (req: Request, res: Response) => {
  ...
  res.json({
    user: {
      ...
      role:
        (doc?.role as string) === 'jastiper' || user.role === 'jastiper' ? 'jastiper' : 'customer',
    },
  });
});
...
router.put('/profile', requireUser, async (req: Request, res: Response) => {
  try {
    ...
    const updated: SessionUser = {
      ...
      selectedArea:
        (update.selectedArea as string) || doc?.selectedArea || user.selectedArea || null,
    };
