...
export default function JastipSummary() {
  ...
  const ongkir = state?.ongkirCustom !== undefined ? Number(state.ongkirCustom) : (isInvoice ? 0 : 10000);
  const biayaLayanan = hitungBiayaLayanan(ongkir);
  const totalSebelumDiskon = isInvoice ? (Number(state?.totalCustom) || (danaBelanja + feeJastip + ongkir)) + biayaLayanan : (danaBelanja + ongkir + biayaLayanan);
  ...
  const pay = async () => {
    ...
    try {
      ...
      const orderDescription = state.item
        ? (state.notes ? `${state.item}\n\nCatatan: ${state.notes}` : state.item)
        : (state.notes || '');
      ...
      const unifiedRoomId = user && user.id < jId ? `room_${user.id}_${jId}` : `room_${jId}_${user?.id}`;
      ...
      if (state.invoiceId) {
        try {
          ...
          if (unifiedRoomId) {
            void API.chat.send(unifiedRoomId, {
      ...
      navigate(`/chat/room?roomId=${unifiedRoomId}`, { 
        state: { 
          room: {
            ...
            lastMessage: state.invoiceId ? `✅ [PEMBAYARAN DITERIMA] Tagihan` : `Halo, saya telah membuat pesanan jastip...`,
      ...
      if ((e as { code?: string }).code === 'INSUFFICIENT_BALANCE') {
        ...
        enqueueSnackbar(`Gagal memproses pembayaran: ${msg}`, { variant: 'error', autoHideDuration: 6000 });
      }
    } finally {
      setPaying(false);
    }
      ...
      {calc ? (
        ...
        <div className="flex-1 max-w-lg w-full mx-auto px-6 py-5 space-y-4 pb-32">
          ...
          <div className="card-pad">
            ...
            <div className="flex justify-between py-1"><span className="text-sm text-ink-secondary">Item</span><span className="text-sm font-medium">{state?.item || '-'}</span></div>
